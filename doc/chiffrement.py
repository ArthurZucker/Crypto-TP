...
def AES_128_CTR(Kaes,Kiv,octets=32):
	try :
...
class ChiffrementWrapperProtocol(ProtocolTransportMixin, NetstringReceiver):
    ...
    def dataReceived(self,data):
        ...
        if(self.DH  == False):
            ...
            Kaes = hashlib.sha256((str(K)+'D').encode()).digest()[:16]
            print(Kaes)
            Kiv  = int(hashlib.sha256((str(K)+'E').encode()).hexdigest()[:16],16)
            Kmac = hashlib.sha256((str(K)+'F').encode()).digest()[:16]
            return super().connectionMade()
        else :
            tag     = data[-32:]
            data    = data[:-32]
            mask = AES_128_CTR(Kaes,Kiv,len(data))
            decrypted_data = bytes_xor(data,mask)
            if(tag == hmac.new(Kmac, msg = decrypted_data, digestmod = hashlib.sha256).digest()) : print("[Chiffrement] TAG OK")
    ...
    def write(self, data):
        ...
        tag = hmac.new( Kmac, msg = data, digestmod = hashlib.sha256).digest()
